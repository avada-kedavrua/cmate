...
1. **Namespaces**: Group rules by their target namespace (e.g., `env`, `config`, `mies_config`). Each unique namespace becomes a `[par <namespace>]` section.
...
3. **Targets**: Each non-env namespace needs a `[targets]` declaration. If the user doesn't specify file formats, default to `'json'`.
